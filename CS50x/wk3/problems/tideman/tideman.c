...
bool is_cycle(int origin, int prev, int curr);
...
void sort_pairs(void)
{
    ...
    for (int i = 0; i < (pair_count - 1); i++)
    {
        ...
        {
            if (strength < (preferences[pairs[j].winner][pairs[j].loser] - preferences[pairs[j].loser][pairs[j].winner]))
            {
...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        ...
        for (int j = 0; j < pair_count; j++)
        {
            ...
            {
                free = false;
                if (!is_cycle(pairs[i].loser, j, pairs[i].winner))
                {
